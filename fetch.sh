...
i=0
while read -r u; do i=$((i+1)); curl -sL -m 180 -A "$UA" -o "out/cap$i.html" -w "cap$i %{http_code} %{size_download} %{url_effective}\n" "$u" | tee -a out/log.txt; sleep 3; done <<'U'
...
for s in proseavalonpointe.com prosehorizonsvillage.com prosestevenspointe.com livetesora.com; do
  curl -sL -m 180 -A "$UA" "https://web.archive.org/cdx/search/cdx?url=$s/*&from=202506&to=202608&fl=timestamp,original,statuscode,digest&filter=statuscode:200&collapse=digest" -o "out/cdx_$s.txt"; echo "cdx $s $(wc -l < out/cdx_$s.txt) rows" | tee -a out/log.txt; sleep 3
done
